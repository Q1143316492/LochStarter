...
#include "Performance/LochPerformanceSettings.h"
#include "DeviceProfiles/DeviceProfileManager.h"
#include "DeviceProfiles/DeviceProfile.h"
#include "HAL/PlatformFramePacer.h"
#include "Development/LochPlatformEmulationSettings.h"
#include "SoundControlBus.h"
#include "AudioModulationStatics.h"
#include "Audio/LochAudioSettings.h"
...
namespace LochSettingsHelpers
{
	...
	void FillScalabilitySettingsFromDeviceProfile(FLochScalabilitySnapshot& Mode, const FString& Suffix = FString())
	{
		static_assert(sizeof(Scalability::FQualityLevels) == 88, "This function may need to be updated to account for new members");
	...
	{
		const ULochPlatformSpecificRenderingSettings* PlatformSettings = ULochPlatformSpecificRenderingSettings::Get();
		...
		int32 LimitIndex = PossibleRates.FindLastByPredicate([=](const int32& TestRate)
		{
			...
			const bool bIsSupported = ULochSettingsLocal::IsSupportedMobileFramePace(TestRate);

			return bAtOrBelowDesiredRate && bQualityDoesntExceedLimit && bIsSupported;
		});

		return PossibleRates.IsValidIndex(LimitIndex) ? PossibleRates[LimitIndex] : ULochSettingsLocal::GetDefaultMobileFrameRate();
	}
...
ULochSettingsLocal::ULochSettingsLocal()
{
	...
	bEnableScalabilitySettings = ULochPlatformSpecificRenderingSettings::Get()->bSupportsGranularVideoQualitySettings;
...
void ULochSettingsLocal::SetToDefaults()
{
	...
	bEnableLatencyTrackingStats = ULochSettingsLocal::DoesPlatformSupportLatencyTrackingStats();
...
void ULochSettingsLocal::LoadSettings(bool bForceReload)
{
	...
	const ULochPlatformSpecificRenderingSettings* PlatformSettings = ULochPlatformSpecificRenderingSettings::Get();
	if (PlatformSettings->FramePacingMode == ELochFramePacingMode::ConsoleStyle)
	{
		FrameRateLimit = 0.0f;
	}
	...
	LochSettingsHelpers::FillScalabilitySettingsFromDeviceProfile(DeviceDefaultScalabilitySettings);
...
float ULochSettingsLocal::GetEffectiveFrameRateLimit()
{
	...
	if (PlatformSettings->FramePacingMode == ELochFramePacingMode::DesktopStyle)
	{
...
int32 ULochSettingsLocal::GetHighestLevelOfAnyScalabilityChannel() const
{
	return LochSettingsHelpers::GetHighestLevelOfAnyScalabilityChannel(ScalabilityQuality);
}

void ULochSettingsLocal::OverrideQualityLevelsToScalabilityMode(const FLochScalabilitySnapshot& InMode, Scalability::FQualityLevels& InOutLevels)
{
	static_assert(sizeof(Scalability::FQualityLevels) == 88, "This function may need to be updated to account for new members");
...
void ULochSettingsLocal::ClampQualityLevelsToDeviceProfile(const Scalability::FQualityLevels& ClampLevels, Scalability::FQualityLevels& InOutLevels)
{
	static_assert(sizeof(Scalability::FQualityLevels) == 88, "This function may need to be updated to account for new members");
...
void ULochSettingsLocal::OnExperienceLoaded()
{
	ReapplyThingsDueToPossibleDeviceProfileChange();
}

void ULochSettingsLocal::OnHotfixDeviceProfileApplied()
{
	ReapplyThingsDueToPossibleDeviceProfileChange();
}

void ULochSettingsLocal::ReapplyThingsDueToPossibleDeviceProfileChange()
{
	ApplyNonResolutionSettings();
}

void ULochSettingsLocal::SetShouldUseFrontendPerformanceSettings(bool bInFrontEnd)
{
	bInFrontEndForPerformancePurposes = bInFrontEnd;
	UpdateEffectiveFrameRateLimit();
}

bool ULochSettingsLocal::ShouldUseFrontendPerformanceSettings() const
{
#if WITH_EDITOR
...
ELochStatDisplayMode ULochSettingsLocal::GetPerfStatDisplayState(ELochDisplayablePerformanceStat Stat) const
{
	if (const ELochStatDisplayMode* pMode = DisplayStatList.Find(Stat))
	{
		return *pMode;
	}
	else
	{
		return ELochStatDisplayMode::Hidden;
	}
}

void ULochSettingsLocal::SetPerfStatDisplayState(ELochDisplayablePerformanceStat Stat, ELochStatDisplayMode DisplayMode)
{
	if (DisplayMode == ELochStatDisplayMode::Hidden)
	{
...
bool ULochSettingsLocal::DoesPlatformSupportLatencyMarkers()
{
	return ICommonUIModule::GetSettings().GetPlatformTraits().HasTag(PerfStatTags::TAG_Platform_Trait_SupportsLatencyMarkers);
}

void ULochSettingsLocal::SetEnableLatencyFlashIndicators(const bool bNewVal)
{
...
void ULochSettingsLocal::SetEnableLatencyTrackingStats(const bool bNewVal)
{
...
void ULochSettingsLocal::ApplyLatencyTrackingStatSetting()
{
...
bool ULochSettingsLocal::DoesPlatformSupportLatencyTrackingStats()
{
	return ICommonUIModule::GetSettings().GetPlatformTraits().HasTag(PerfStatTags::TAG_Platform_Trait_SupportsLatencyStats);
}

float ULochSettingsLocal::GetDisplayGamma() const
{
	return DisplayGamma;
}

void ULochSettingsLocal::SetDisplayGamma(float InGamma)
{
	DisplayGamma = InGamma;
	ApplyDisplayGamma();
}

void ULochSettingsLocal::ApplyDisplayGamma()
{
	if (GEngine)
	{
...
float ULochSettingsLocal::GetFrameRateLimit_OnBattery() const
{
	return FrameRateLimit_OnBattery;
}

void ULochSettingsLocal::SetFrameRateLimit_OnBattery(float NewLimitFPS)
{
	FrameRateLimit_OnBattery = NewLimitFPS;
	UpdateEffectiveFrameRateLimit();
}

float ULochSettingsLocal::GetFrameRateLimit_InMenu() const
{
	return FrameRateLimit_InMenu;
}

void ULochSettingsLocal::SetFrameRateLimit_InMenu(float NewLimitFPS)
{
	FrameRateLimit_InMenu = NewLimitFPS;
	UpdateEffectiveFrameRateLimit();
}

float ULochSettingsLocal::GetFrameRateLimit_WhenBackgrounded() const
{
	return FrameRateLimit_WhenBackgrounded;
}

void ULochSettingsLocal::SetFrameRateLimit_WhenBackgrounded(float NewLimitFPS)
{
	FrameRateLimit_WhenBackgrounded = NewLimitFPS;
	UpdateEffectiveFrameRateLimit();
}

float ULochSettingsLocal::GetFrameRateLimit_Always() const
{
	return GetFrameRateLimit();
}

void ULochSettingsLocal::SetFrameRateLimit_Always(float NewLimitFPS)
{
	SetFrameRateLimit(NewLimitFPS);
	UpdateEffectiveFrameRateLimit();
}

void ULochSettingsLocal::UpdateEffectiveFrameRateLimit()
{
...
int32 ULochSettingsLocal::GetDefaultMobileFrameRate()
{
	return CVarDeviceProfileDrivenMobileDefaultFrameRate.GetValueOnGameThread();
}

int32 ULochSettingsLocal::GetMaxMobileFrameRate()
{
	return CVarDeviceProfileDrivenMobileMaxFrameRate.GetValueOnGameThread();
}

bool ULochSettingsLocal::IsSupportedMobileFramePace(int32 TestFPS)
{
...
int32 ULochSettingsLocal::GetFirstFrameRateWithQualityLimit() const
{
	return LochSettingsHelpers::GetFirstFrameRateWithQualityLimit();
}

int32 ULochSettingsLocal::GetLowestQualityWithFrameRateLimit() const
{
	return LochSettingsHelpers::GetLowestQualityWithFrameRateLimit();
}

void ULochSettingsLocal::ResetToMobileDeviceDefaults()
{
	// Reset frame rate
...
int32 ULochSettingsLocal::GetMaxSupportedOverallQualityLevel() const
{
	const ULochPlatformSpecificRenderingSettings* PlatformSettings = ULochPlatformSpecificRenderingSettings::Get();
	if ((PlatformSettings->FramePacingMode == ELochFramePacingMode::MobileStyle) && DeviceDefaultScalabilitySettings.bHasOverrides)
	{
		return LochSettingsHelpers::GetHighestLevelOfAnyScalabilityChannel(DeviceDefaultScalabilitySettings.Qualities);
	}
	else
	{
		return 3;
...
void ULochSettingsLocal::SetMobileFPSMode(int32 NewLimitFPS)
{
	const ULochPlatformSpecificRenderingSettings* PlatformSettings = ULochPlatformSpecificRenderingSettings::Get();
	if (PlatformSettings->FramePacingMode == ELochFramePacingMode::MobileStyle)
	{
...
void ULochSettingsLocal::SetDesiredMobileFrameRateLimit(int32 NewLimitFPS)
{
...
void ULochSettingsLocal::ClampMobileFPSQualityLevels(bool bWriteBack)
{
	const ULochPlatformSpecificRenderingSettings* PlatformSettings = ULochPlatformSpecificRenderingSettings::Get();
	if (PlatformSettings->FramePacingMode == ELochFramePacingMode::MobileStyle)
	{
		const int32 QualityLimit = LochSettingsHelpers::GetApplicableOverallQualityLimit(DesiredMobileFrameRateLimit);
...
void ULochSettingsLocal::ClampMobileQuality()
{
	const ULochPlatformSpecificRenderingSettings* PlatformSettings = ULochPlatformSpecificRenderingSettings::Get();
	if (PlatformSettings->FramePacingMode == ELochFramePacingMode::MobileStyle)
	{
...
void ULochSettingsLocal::ClampMobileResolutionQuality(int32 TargetFPS)
{
	// Clamp mobile resolution quality
	float MaxMobileResQuality = LochSettingsHelpers::GetApplicableResolutionQualityLimit(TargetFPS);
...
void ULochSettingsLocal::RemapMobileResolutionQuality(int32 FromFPS, int32 ToFPS)
{
	...
	float MaxScaleValue = 0.0f;
	GetResolutionScaleInformationEx(CurrentScaleNormalized, CurrentScaleValue, MinScaleValue, MaxScaleValue);
	float FromMaxMobileResQuality = LochSettingsHelpers::GetApplicableResolutionQualityLimit(FromFPS);
	float ToMaxMobileResQuality = LochSettingsHelpers::GetApplicableResolutionQualityLimit(ToFPS);
...
FString ULochSettingsLocal::GetDesiredDeviceProfileQualitySuffix() const
{
	return DesiredUserChosenDeviceProfileSuffix;
}

void ULochSettingsLocal::SetDesiredDeviceProfileQualitySuffix(const FString& InDesiredSuffix)
{
	DesiredUserChosenDeviceProfileSuffix = InDesiredSuffix;
}

void ULochSettingsLocal::SetHeadphoneModeEnabled(bool bEnabled)
{
...
bool ULochSettingsLocal::IsHeadphoneModeEnabled() const
{
	return bUseHeadphoneMode;
}

bool ULochSettingsLocal::CanModifyHeadphoneModeEnabled() const
{
	static IConsoleVariable* BinauralSpatializationDisabledCVar = IConsoleManager::Get().FindConsoleVariable(TEXT("au.DisableBinauralSpatialization"));
	...
	const bool bBinauralSettingControlledByOS = LochSettingsHelpers::HasPlatformTrait(TAG_Platform_Trait_BinauralSettingControlledByOS);

	return bHRTFOptionAvailable && !bBinauralSettingControlledByOS;
}

bool ULochSettingsLocal::IsHDRAudioModeEnabled() const
{
	return bUseHDRAudioMode;
}

void ULochSettingsLocal::SetHDRAudioModeEnabled(bool bEnabled)
{
	...
	if (GEngine)
	{
		...
		{
			if (ULochAudioMixEffectsSubsystem* LochAudioMixEffectsSubsystem = World->GetSubsystem<ULochAudioMixEffectsSubsystem>())
			{
				LochAudioMixEffectsSubsystem->ApplyDynamicRangeEffectsChains(bEnabled);
			}
		}
	}
}

bool ULochSettingsLocal::CanRunAutoBenchmark() const
{
	const ULochPlatformSpecificRenderingSettings* PlatformSettings = ULochPlatformSpecificRenderingSettings::Get();
	return PlatformSettings->bSupportsAutomaticVideoQualityBenchmark;
}

bool ULochSettingsLocal::ShouldRunAutoBenchmarkAtStartup() const
{
...
void ULochSettingsLocal::RunAutoBenchmark(bool bSaveImmediately)
{
...
void ULochSettingsLocal::ApplyScalabilitySettings()
{
	Scalability::SetQualityLevels(ScalabilityQuality);
}

float ULochSettingsLocal::GetOverallVolume() const
{
	return OverallVolume;
}

void ULochSettingsLocal::SetOverallVolume(float InVolume)
{
...
float ULochSettingsLocal::GetMusicVolume() const
{
	return MusicVolume;
}

void ULochSettingsLocal::SetMusicVolume(float InVolume)
{
	// Cache the incoming volume value
...
float ULochSettingsLocal::GetSoundFXVolume() const
{
	return SoundFXVolume;
}

void ULochSettingsLocal::SetSoundFXVolume(float InVolume)
{
	// Cache the incoming volume value
...
float ULochSettingsLocal::GetDialogueVolume() const
{
	return DialogueVolume;
}

void ULochSettingsLocal::SetDialogueVolume(float InVolume)
{
	// Cache the incoming volume value
...
float ULochSettingsLocal::GetVoiceChatVolume() const
{
	return VoiceChatVolume;
}

void ULochSettingsLocal::SetVoiceChatVolume(float InVolume)
{
	// Cache the incoming volume value
...
void ULochSettingsLocal::SetVolumeForControlBus(USoundControlBus* InSoundControlBus, float InVolume)
{
	// Check to see if references to the control buses and control bus mixes have been loaded yet
...
void ULochSettingsLocal::SetAudioOutputDeviceId(const FString& InAudioOutputDeviceId)
{
	AudioOutputDeviceId = InAudioOutputDeviceId;
	OnAudioOutputDeviceChanged.Broadcast(InAudioOutputDeviceId);
}

void ULochSettingsLocal::ApplySafeZoneScale()
{
...
void ULochSettingsLocal::SetOverallScalabilityLevel(int32 Value)
{
	...
	if (PlatformSettings->FramePacingMode == ELochFramePacingMode::MobileStyle)
	{
		...
		const int32 ConstrainedFrameRateLimit = LochSettingsHelpers::ConstrainFrameRateToBeCompatibleWithOverallQuality(DesiredMobileFrameRateLimit, Value);
...
void ULochSettingsLocal::SetControllerPlatform(const FName InControllerPlatform)
{
...
FName ULochSettingsLocal::GetControllerPlatform() const
{
	return ControllerPlatform;
}

void ULochSettingsLocal::LoadUserControlBusMix()
{
	if (GEngine)
	{
		if (const UWorld* World = GEngine->GetCurrentPlayWorld())
		{
			if (const ULochAudioSettings* LochAudioSettings = GetDefault<ULochAudioSettings>())
			{
				...
				if (UObject* ObjPath = LochAudioSettings->UserSettingsControlBusMix.TryLoad())
				{
					...
					{
						ensureMsgf(SoundControlBusMix, TEXT("User Settings Control Bus Mix reference missing from Loch Audio Settings."));
					}
				}
			}
		}
...
void ULochSettingsLocal::OnAppActivationStateChanged(bool bIsActive)
{
	// We might want to adjust the frame rate when the app loses/gains focus on multi-window platforms
	UpdateEffectiveFrameRateLimit();
}

void ULochSettingsLocal::UpdateGameModeDeviceProfileAndFps()
{
#if WITH_EDITOR
	...
	const ULochPlatformSpecificRenderingSettings* PlatformSettings = ULochPlatformSpecificRenderingSettings::Get();
	const TArray<FLochQualityDeviceProfileVariant>& UserFacingVariants = PlatformSettings->UserFacingDeviceProfileOptions;
	...
	int32 SuffixIndex = UserFacingVariants.IndexOfByPredicate([&](const FLochQualityDeviceProfileVariant& Data){ return Data.DeviceProfileSuffix == UserChosenDeviceProfileSuffix; });
	...
	if (GIsEditor)
	{
		const ULochPlatformEmulationSettings* Settings = GetDefault<ULochPlatformEmulationSettings>();
	...
	{
	case ELochFramePacingMode::MobileStyle:
		UpdateMobileFramePacing();
		break;
	case ELochFramePacingMode::ConsoleStyle:
		UpdateConsoleFramePacing();
		break;
	case ELochFramePacingMode::DesktopStyle:
		UpdateDesktopFramePacing();
		break;
	}
}

void ULochSettingsLocal::UpdateConsoleFramePacing()
{
...
void ULochSettingsLocal::UpdateDesktopFramePacing()
{
...
void ULochSettingsLocal::UpdateMobileFramePacing()
{
...
void ULochSettingsLocal::UpdateDynamicResFrameTime(float TargetFPS)
{
