...
public:
	...
	ELochStatDisplayMode GetPerfStatDisplayState(ELochDisplayablePerformanceStat Stat) const;
	
	/** Sets the display mode for the specified performance stat */
	void SetPerfStatDisplayState(ELochDisplayablePerformanceStat Stat, ELochStatDisplayMode DisplayMode);

	/** Fired when the display state for a performance stat has changed, or the settings are applied */
	DECLARE_EVENT(ULochSettingsLocal, FPerfStatSettingsChanged);
	...
	DECLARE_EVENT(ULochSettingsLocal, FLatencyFlashInidicatorSettingChanged);
	UFUNCTION()
	...
	DECLARE_EVENT(ULochSettingsLocal, FLatencyStatEnabledSettingChanged);
...
private:
	...
	// 在HUD中显示的统计信息列表
	UPROPERTY(Config)
	TMap<ELochDisplayablePerformanceStat, ELochStatDisplayMode> DisplayStatList;

	// 用于显示统计小部件容器绑定的事件
	FPerfStatSettingsChanged PerfStatSettingsChangedEvent;

	// 如果为 true，将启用延迟闪烁标记，该标记可用于测量输入延迟。
	UPROPERTY(Config)
	bool bEnableLatencyFlashIndicators = false;

	// 用于当玩家输入要绑定的延迟闪烁指示器设置发生更改时的事件。
	FLatencyFlashInidicatorSettingChanged LatencyFlashInidicatorSettingsChangedEvent;

	// 延迟统计信息的开启或关闭状态发生变化时的事件
	FLatencyStatEnabledSettingChanged LatencyStatIndicatorSettingsChangedEvent;

	//如果为 true，则游戏将通过 ILatencyMarkerModule 模块跟踪延迟统计数据。
	// 这使您能够查看更多面向延迟的性能统计数据。
	// 如果平台支持，默认值设置为 true，否则为 false。
	UPROPERTY(Config)
...
private:
	...
	void OverrideQualityLevelsToScalabilityMode(const FLochScalabilitySnapshot& InMode, Scalability::FQualityLevels& InOutLevels);
...
private:
	...
	FLochScalabilitySnapshot DeviceDefaultScalabilitySettings;
