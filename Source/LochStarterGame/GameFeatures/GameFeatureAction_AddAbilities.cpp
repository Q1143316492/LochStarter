...
#include "Player/LochPlayerState.h" //@TODO: For the fname
...
EDataValidationResult UGameFeatureAction_AddAbilities::IsDataValid(FDataValidationContext& Context) const
{
	...
	for (const FGameFeatureAbilitiesEntry& Entry : AbilitiesList)
	{
		...
		for (const FLochAbilityGrant& Ability : Entry.GrantedAbilities)
		{
		...
		for (const FLochAttributeSetGrant& Attributes : Entry.GrantedAttributes)
		{
		...
		for (const TSoftObjectPtr<const ULochAbilitySet>& AttributeSetPtr : Entry.GrantedAbilitySets)
		{
...
void UGameFeatureAction_AddAbilities::HandleActorExtension(AActor* Actor, FName EventName, int32 EntryIndex, FGameFeatureStateChangeContext ChangeContext)
{
	...
	if (AbilitiesList.IsValidIndex(EntryIndex) && ActiveData)
	{
		...
		else if ((EventName == UGameFrameworkComponentManager::NAME_ExtensionAdded) || (EventName == ALochPlayerState::NAME_LochAbilityReady))
		{
...
void UGameFeatureAction_AddAbilities::AddActorAbilities(AActor* Actor, const FGameFeatureAbilitiesEntry& AbilitiesEntry, FPerContextData& ActiveData)
{
	...
	if (UAbilitySystemComponent* AbilitySystemComponent = FindOrAddComponentForActor<UAbilitySystemComponent>(Actor, AbilitiesEntry, ActiveData))
	{
		...
		for (const FLochAbilityGrant& Ability : AbilitiesEntry.GrantedAbilities)
		{
			if (!Ability.AbilityType.IsNull())
			{
		...
		for (const FLochAttributeSetGrant& Attributes : AbilitiesEntry.GrantedAttributes)
		{
			if (!Attributes.AttributeSetType.IsNull())
			{
		...
		ULochAbilitySystemComponent* LochASC = CastChecked<ULochAbilitySystemComponent>(AbilitySystemComponent);
		for (const TSoftObjectPtr<const ULochAbilitySet>& SetPtr : AbilitiesEntry.GrantedAbilitySets)
		{
			if (const ULochAbilitySet* Set = SetPtr.Get())
			{
				Set->GiveToAbilitySystem(LochASC, &AddedExtensions.AbilitySetHandles.AddDefaulted_GetRef());
			}
		}
...
void UGameFeatureAction_AddAbilities::RemoveActorAbilities(AActor* Actor, FPerContextData& ActiveData)
{
	if (FActorExtensions* ActorExtensions = ActiveData.ActiveExtensions.Find(Actor))
	{
		if (UAbilitySystemComponent* AbilitySystemComponent = Actor->FindComponentByClass<UAbilitySystemComponent>())
		{
			...
			ULochAbilitySystemComponent* LochASC = CastChecked<ULochAbilitySystemComponent>(AbilitySystemComponent);
			for (FLochAbilitySet_GrantedHandles& SetHandle : ActorExtensions->AbilitySetHandles)
			{
				SetHandle.TakeFromAbilitySystem(LochASC);
			}
		}
