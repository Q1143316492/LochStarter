...
#include "LochAudioSettings.generated.h"
...
USTRUCT()
struct FLochSubmixEffectChainMap
{
	GENERATED_BODY()
 ...
 * 
 */
UCLASS(MinimalAPI, config = Game, defaultconfig, meta = (DisplayName = "LochAudioSettings"))
class ULochAudioSettings : public UDeveloperSettings
{
	GENERATED_BODY()
...
public:
	...
	TArray<FLochSubmixEffectChainMap> HDRAudioSubmixEffectChain;
	
	/** Submix Processing Chains to achieve low dynamic range audio output */
	UPROPERTY(config, EditAnywhere, Category = EffectSettings)
	TArray<FLochSubmixEffectChainMap> LDRAudioSubmixEffectChain;
