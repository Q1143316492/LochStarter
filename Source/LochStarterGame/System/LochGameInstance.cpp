...
#include UE_INLINE_GENERATED_CPP_BY_NAME(LochGameInstance)

namespace Loch
{
    // 开加密
	static bool bTestEncryption = false;
	static FAutoConsoleVariableRef CVarLochTestEncryption(
		TEXT("Loch.TestEncryption"),
		bTestEncryption,
	...
	static FAutoConsoleVariableRef CVarLochUseDTLSEncryption(
		TEXT("Loch.UseDTLSEncryption"),
		bUseDTLSEncryption,
		TEXT("Set to true if using Loch.TestEncryption and the DTLS packet handler."),
		ECVF_Default);
	...
	static FAutoConsoleVariableRef CVarLochTestDTLSFingerprint(
		TEXT("Loch.TestDTLSFingerprint"),
...
ULochGameInstance::ULochGameInstance(const FObjectInitializer& ObjectInitializer)
	: Super(ObjectInitializer)
{
}

void ULochGameInstance::Init()
{
	Super::Init();
	...
	{
		ComponentManager->RegisterInitState(LochGameplayTags::InitState_Spawned, false, FGameplayTag());
		ComponentManager->RegisterInitState(LochGameplayTags::InitState_DataAvailable, false, LochGameplayTags::InitState_Spawned);
		ComponentManager->RegisterInitState(LochGameplayTags::InitState_DataInitialized, false, LochGameplayTags::InitState_DataAvailable);
		ComponentManager->RegisterInitState(LochGameplayTags::InitState_GameplayReady, false, LochGameplayTags::InitState_DataInitialized);
	}
	...
	{
		SessionSubsystem->OnPreClientTravelEvent.AddUObject(this, &ULochGameInstance::OnPreClientTravelToSession);
	}
}

void ULochGameInstance::Shutdown()
{
	if (UCommonSessionSubsystem* SessionSubsystem = GetSubsystem<UCommonSessionSubsystem>())
	{
...
ALochPlayerController* ULochGameInstance::GetPrimaryPlayerController() const
{
	return Cast<ALochPlayerController>(Super::GetPrimaryPlayerController(false));
}

bool ULochGameInstance::CanJoinRequestedSession() const
{
...
void ULochGameInstance::HandlerUserInitialized(const UCommonUserInfo* UserInfo, bool bSuccess, FText Error, ECommonUserPrivilege RequestedPrivilege, ECommonUserOnlineContext OnlineContext)
{
	...
	{
        // TODO cwl
		// ULochLocalPlayer* LocalPlayer = Cast<ULochLocalPlayer>(GetLocalPlayerByIndex(UserInfo->LocalPlayerIndex));
...
void ULochGameInstance::ReceivedNetworkEncryptionToken(const FString& EncryptionToken, const FOnEncryptionKeyResponse& Delegate)
{
	...
	{
#if UE_WITH_DTLS
		if (Loch::bUseDTLSEncryption)
		{
			TSharedPtr<FDTLSCertificate> Cert;

			if (Loch::bTestDTLSFingerprint)
			{
			...
			else
			{
				...
				const FString CertPath = FPaths::ProjectContentDir() / TEXT("DTLS") / TEXT("LochTest.pem");
...
void ULochGameInstance::ReceivedNetworkEncryptionAck(const FOnEncryptionKeyResponse& Delegate)
{
	// This is a simple implementation to demonstrate using encryption for game traffic using a hardcoded key.
	// For a complete implementation, you would likely want to retrieve the encryption key from a secure source,
	// such as from a web service over HTTPS. This could be done in this function, even asynchronously - just
...
void ULochGameInstance::OnPreClientTravelToSession(FString& URL)
{
	// Add debug encryption token if desired.
	if (Loch::bTestEncryption)
	{
#if UE_WITH_DTLS
		if (Loch::bUseDTLSEncryption)
		{
